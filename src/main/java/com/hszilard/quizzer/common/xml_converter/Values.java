package main.java.com.hszilard.quizzer.common.xml_converter;

/**
 * @author Szilárd Hompoth at https://github.com/hszilard93
 */
final class Values {
    /* The XML tags and attribute names describing a Quiz object.*/
    static final String QUIZ_TAG     = "quiz";
    static final String CREATED_ATTR = "created";
    static final String EDITED_ATTR  = "edited";
    static final String TITLE_TAG    = "title";
    static final String QUESTION_TAG = "question";
    static final String QUESTION_TEXT_TAG = "text";
    static final String ANSWER_TAG   = "answer";
    static final String CORRECT_ATTR = "correct";
    static final String ANSWER_TEXT_TAG = "text";
}
